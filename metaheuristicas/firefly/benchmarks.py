...
# Implementar el FA
kwargs = {'func': sopt.rosen, 'dim': 2, 'tam_pob': 30, 'alpha': 0.9,
'beta': 0.2, 'gamma': 1.0, 'inf': -5.0, 'sup': 10.0}
...
print(f'Python: {res.x}')
# Implementar el FA
fa_optim = fa.FAOpt(sphere, 256, 40, 0.9, 0.2, 1.0, -5.12, 5.12)
...
print(f'Python: {res.x}')
# Implementar el FA
fa_optim = fa.FAOpt(beale, 2, 25, 0.9, 0.2, 1.0, -4.5, 4.5)
...
def gp(x):
    term1 = (x[0] + x[1] + 1.0)**2
    term2 = 19.0 - 14.0*x[0] + 3.0*x[0]**2 - 14.0*x[1] + 6.0*x[0]*x[1] + 3.0*x[1]**2
    term3 = (2.0*x[0] - 3.0*x[1])**2
    term4 = 18.0 - 32.0*x[0] + 12.0*x[0]**2 + 48.0*x[1] - 36.0*x[0]*x[1] + 27.0*x[1]**2
...
print(f'Python: {res.x}')
# Implementar el FA
fa_optim = fa.FAOpt(gp, 2, 25, 0.9, 0.2, 1.0, -2.0, 2.0)
